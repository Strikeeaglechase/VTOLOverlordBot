...
class Query extends Command {
	...
	async run({ message, framework, app }: CommandEvent<Application>, query: string) {
		const lobby = app.lobbies.find(l => l.name.toLowerCase().startsWith(query.toLowerCase()));
		...
		if (parseGameVersion(lobby.gameVersion) == "Modded") {
			emb.addField(`Mods (${lobby.modCount})`, lobby.loadedMods.length > 0 ? lobby.loadedMods.join("\n") : "No mods loaded (why are they even playing on modded?)");
		}
		return emb;
	}
}
